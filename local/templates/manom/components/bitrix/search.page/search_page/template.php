...
$uri->addParams(array("search_sort"=>"1"));
$urlSort1 = $uri->getUri();
$uri->addParams(array("search_sort"=>"2"));
$urlSort2 = $uri->getUri();
?>
...
<section class="catalog-block ">
        ...
        <div id='PROPDS_BLOCK'>
            <div class="cb-line no-gutters ffdf" style="display: flex;">
                    <?foreach ($arResult['SEARCH'] as $key => $arItems) {?>
                            ...
                            $arItems['CATALOG_QUANTITY'] = $quantity = $nearestQuantity = CCatalogProduct::GetNearestQuantityPrice($arItems['ID'], 1, $USER->GetUserGroupArray());
                            ...
                            $morePhoto = array();
                            $sellProd = false;
                            $getMorePhoto = CIBlockElement::GetProperty(7, $arItems['ID'], "sort", "asc", array("CODE" => "MORE_PHOTO"));
                            while ($resMorePhoto = $getMorePhoto->Fetch()) {
                                $morePhoto[] = $resMorePhoto['VALUE'];
                            }
                            $getProp = CIBlockElement::GetProperty(7, $arItems['ID'], "sort", "asc", array("CODE" => "SELL_PROD"));
                            ...
                            <div class="cb-line__item">
                                    <div class="product-card cb-line-card <?=$arItems['CATALOG_QUANTITY'] > 0 || $arCanBuy == 1 ? 'enable' : 'disable'?>">  <!-- disable -->
                                            ...
                                            <div class="cb-line-card__main">
                                                    ...
                                                    <div class="cb-line-nav-top">
                                                            ...
                                                            <label>
                                                                ...
                                                                <div class="addToFavoriteListOnFP cb-line-nav-top__favorite addToFavoriteList <?=!checkProdInFavoriteAndCompareList($arPrice['PRODUCT_ID'], 'UF_FAVORITE_ID') ? 'notActive' : '';?>" data-id='<?=$arPrice['PRODUCT_ID']?>'>В&nbsp;избранное</div>
                                                            </label>
                                                            <div class="cb-line-nav-top__list addToCompareList <?=!checkProdInFavoriteAndCompareList($arPrice['PRODUCT_ID'], 'UF_COMPARE_ID') ? 'notActive' : '';?>" data-id='<?=$arPrice['PRODUCT_ID']?>'>Сравнить</div>
                                                            <!-- <label>
                                            ...
                                            <div class="p-nav-bottom cb-line-bottom">
                                                    ...
                                                    <button class="cb-line-bottom__buy <?=$arItems['CATALOG_QUANTITY'] > 0 || $arCanBuy == 1 ? 'addToCartBtn' : ''?>" data-id='<?=$arPrice['PRODUCT_ID']?>' <?=$arItems['CATALOG_QUANTITY'] > 0 || $arCanBuy == 1 ? 'enable' : 'disable'?>>Купить</button>
                                            </div>
                                    </div>
                            </div>
                    <?}?>
    ...
    <?}else{?>
        <div class="message-incorrect" style="text-align:center;">
            <p>Упс 🙁 По запросу «<?=$_REQUEST['q']?>» ничего не нашли</p><br>
