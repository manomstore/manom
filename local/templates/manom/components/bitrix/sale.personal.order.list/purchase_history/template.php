...
else
{
	...
	?>
	<div class="row mb-3 order-history-top_table">
        <div class="col-4">
            <a class="order-history-back" href="/user/profile.php"><i class="fas fa-chevron-left"></i> Вернуться в личный кабинет</a>
        </div>
		<div class="col" style="text-align: right;">
			...
			{
				?>
				<a class="sale-order-history-link" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_VIEW_ORDERS_HISTORY")?></a>
				<?
			}
			if ($_REQUEST["filter_history"] == 'Y')
			{
				?>
				<a class="sale-order-history-link" href="<?=$APPLICATION->GetCurPageParam("", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_CUR_ORDERS")?></a>
				<?
				if ($_REQUEST["show_canceled"] == 'Y')
				{
					?>
				...
				{
					?>
					<a class="sale-order-history-link" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y&show_canceled=Y", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_VIEW_ORDERS_CANCELED")?></a>
					<?
				}
			}
			?>
	...
	if (!count($arResult['ORDERS']))
	{
		...
		<div class="row mb-3">
			<div class="col">
				<a href="<?=htmlspecialcharsbx($arParams['PATH_TO_CATALOG'])?>" class="sale-order-history-link"><?=Loc::getMessage('SPOL_TPL_LINK_TO_CATALOG')?></a>
			</div>
		</div>
		<?
	}
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $arResult['SORT_TYPE'] == 'STATUS')
			{
				...
				<div class="row mb-3">
					<div class="col">
						<h2 class="order-history-h2"><?= Loc::getMessage('SPOL_TPL_ORDER_IN_STATUSES') ?> &laquo;<?=htmlspecialcharsbx($arResult['INFO']['STATUS'][$orderHeaderStatus]['NAME'])?>&raquo;</h2>
					</div>
				</div>
				<?
			}
			?>
			<div class="row sale-order-list-title-container">
				<h3 class="col g-font-size-20 mb-1 mt-1 order-history-h3">
